...
serve(async (req) => {
    ...
    try {
        ...
        if (!RESEND_API_KEY) {
            // Dev mode: log the OTP to console
...
<head>
  ...
  <title>Código de Verificação - NexFit</title>
</head>
    ...
    <tr>
      <td align="center">
        <table width="480" cellpadding="0" cellspacing="0" style="background:#111;border-radius:16px;border:1px solid #222;overflow:hidden;">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#22c55e,#16a34a);padding:32px;text-align:center;">
              <h1 style="margin:0;color:#fff;font-size:28px;font-weight:900;letter-spacing:-0.5px;">NexFit</h1>
          ...
          <tr>
            <td style="padding:40px 32px;">
              ...
              <p style="margin:0 0 32px;color:#aaa;font-size:15px;line-height:1.6;">
                Use o código abaixo para verificar seu endereço de e-mail e ativar sua conta NexFit.
              </p>
              <!-- OTP Code -->
              ...
              <div style="background:#1a1a1a;border-left:3px solid #f59e0b;border-radius:4px;padding:12px 16px;margin-bottom:24px;">
                <p style="margin:0;color:#888;font-size:13px;">
                  ⚠️ Se você não criou uma conta no NexFit, ignore este e-mail.
                </p>
              </div>
              <p style="margin:0;color:#555;font-size:12px;line-height:1.6;">
                Este código expira em 15 minutos. Caso precise de um novo código, acesse o aplicativo e clique em "Reenviar código".
              </p>
            </td>
          </tr>
          <!-- Footer -->
          <tr>
            <td style="background:#0d0d0d;padding:20px 32px;border-top:1px solid #1a1a1a;text-align:center;">
        ...
        const resendRes = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
                from: "NexFit <onboarding@resend.dev>",
                to: [email],
                subject: `${otpCode} — Seu código de verificação NexFit`,
                html: emailHtml,
            }),
        });
        ...
        if (!resendRes.ok) {
            ...
            console.error("send-email-otp: Resend error", resendError);
            return json({ error: "Falha ao enviar e-mail. Tente novamente." }, { status: 500 });
        }
