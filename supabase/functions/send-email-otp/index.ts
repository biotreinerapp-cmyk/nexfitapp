...
serve(async (req) => {
    ...
    const RESEND_FROM_EMAIL = Deno.env.get("RESEND_FROM_EMAIL"); // e.g. "NexFit <noreply@aurevohub.com.br>"
    ...
    try {
        ...
        if (RESEND_API_KEY && RESEND_FROM_EMAIL) {
            ...
            return json({ error: `Falha ao enviar e-mail via Resend: ${resendError}` }, { status: 500 });
        }

        return json({ error: "Resend não configurado corretamente (API Key ou From Email ausentes)" }, { status: 500 });
