...
const ProfessionalDestaquePage = () => {
    ...
    useEffect(() => {
        document.title = "Destaque seu Perfil - Nexfit";
    }, []);
    ...
    useEffect(() => {
        const load = async () => {
            ...
            // Load offers (using the same highlight_offers table)
            ...
            if (pData) {
                setProfessionalData(pData as ProfessionalData);
            }
    ...
    const isAdsActive = () => {
        ...
        if (!professionalData.is_highlighted) return false;
        if (!professionalData.highlight_expires_at) return false;

        // Check if expires_at is in the future
        return isAfter(new Date(professionalData.highlight_expires_at), new Date());
    };
        ...
        <main className="relative min-h-screen overflow-hidden bg-black px-4 pb-28 pt-8 safe-bottom-floating-nav">
            ...
            <div className="relative z-10">
                <header className="mb-6 flex items-center gap-3">
                    <BackIconButton onClick={() => navigate("/professional/dashboard")} />
                ...
                {active && professionalData ? (
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="relative overflow-hidden rounded-[24px] border border-primary/20 bg-primary/5 p-6 backdrop-blur-xl text-center">
                            ...
                            <h2 className="text-xl font-black text-white uppercase tracking-tight">Perfil em Destaque</h2>
                            <p className="mt-2 text-sm text-zinc-400">
                                Seu perfil está aparecendo no topo para os alunos da plataforma.
                            </p>
                            ...
                            <div className="mt-6 inline-flex items-center gap-2 rounded-full bg-black/50 px-4 py-2 border border-white/5">
                                ...
                                <span className="text-xs font-bold text-zinc-300 uppercase tracking-widest">
                                    Válido até: <span className="text-white">{format(new Date(professionalData.highlight_expires_at!), "dd 'de' MMM", { locale: ptBR })}</span>
                                </span>
                            </div>
                        </div>
                            ...
                            <div className="rounded-[24px] border border-white/5 bg-white/[0.03] p-5 backdrop-blur-xl relative overflow-hidden group hover:border-white/10 transition-colors">
                                ...
                                <div className="relative z-10">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Visualizações</span>
                                    </div>
                                    <p className="text-4xl font-black text-white">{professionalData.highlight_clicks}</p>
                                </div>
                            </div>
                            ...
                            <div className="rounded-[24px] border border-white/5 bg-white/[0.03] p-5 backdrop-blur-xl relative overflow-hidden group hover:border-white/10 transition-colors">
                                ...
                                <div className="relative z-10">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Contratações</span>
                                    </div>
                                    <p className="text-4xl font-black text-white">{professionalData.highlight_sales}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="rounded-[24px] border border-white/5 bg-white/[0.03] p-6 text-center backdrop-blur-xl">
                            <p className="text-xs text-zinc-500 leading-relaxed">
                                As métricas de visualizações representam os alunos que abriram seu seu perfil a partir do banner de destaque.
                                Contratações contabilizam as solicitações de serviço por esses alunos.
                            </p>
                            <Button
                                variant="outline"
...
export default ProfessionalDestaquePage;
