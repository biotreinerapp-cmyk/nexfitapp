...
function FoodItemCard({
    ...
    return (
        <div className="rounded-2xl border border-white/5 bg-white/[0.03] overflow-hidden">
            ...
            >
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-500/20 text-[11px] font-black text-emerald-500">
                    {index + 1}
                </span>
...
export default function PlanoAlimentarFormPage() {
    ...
    const [meals, setMeals] = useState<DietMealBlock[]>([emptyMealBlock(0)]);
    ...
    useEffect(() => {
        if (existing) {
            ...
            setCreatorId(existing.professional_creator_id);
    ...
    const saveMutation = useMutation({
        mutationFn: async () => {
            ...
            const payload = {
                ...
                user_id: user!.id,
                is_template: false,
                professional_creator_id: creatorId
            };
            if (isEdit) {
        ...
        onSuccess: () => {
            ...
            toast({ title: isEdit ? "Plano atualizado com sucesso!" : "Plano criado com sucesso!" });
            navigate("/aluno/plano-alimentar");
        },
        onError: (err: any) => {
            toast({ title: err?.message || "Erro ao salvar plano.", variant: "destructive" });
        },
    });
        ...
        <div className="min-h-screen bg-black pb-44">
            ...
            <main className="mx-auto max-w-lg px-4 pt-5 space-y-6">
                    ...
                    <div className="rounded-3xl border border-white/5 bg-white/[0.03] p-4 space-y-4">
                        <div>
                            ...
                            <Input
                                ...
                                className="bg-black/40 border-white/10 text-white rounded-xl h-11 focus-visible:ring-emerald-500 placeholder:text-zinc-600 font-bold"
                            />
                        </div>
                        ...
                        <div className="flex items-center justify-between border-t border-white/5 pt-4">
                            <div>
                                <p className="text-sm font-bold text-white">Plano ativo</p>
                                <p className="text-[10px] text-zinc-500">Apenas um pode ser ativo por vez</p>
                            </div>
                            <Switch
                                id="plan-active"
                                checked={isActive}
                ...
                <section className="space-y-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">
                            Refeições (Dias/Refeição)
                        </h2>
                        <button
                            className="flex items-center gap-1.5 text-[10px] font-black uppercase text-emerald-500 hover:opacity-80 transition-all font-mono"
                            ...
                            <div key={meal.id} className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                <div className="flex items-center gap-2">
                                    <Input
                                        ...
                                        className="bg-black/40 border-white/10 text-white rounded-xl h-10 flex-1 focus-visible:ring-emerald-500 placeholder:text-zinc-600 font-bold"
                                    />
                                    {meals.length > 1 && (
                                        <button
                                ...
                                <button
                                    className="w-full rounded-2xl border border-dashed border-white/10 py-3 text-[11px] font-black uppercase text-zinc-500 hover:border-emerald-500/40 hover:text-emerald-500 transition-all flex items-center justify-center gap-2"
                                    onClick={() => addFoodItem(meal.id)}
                                >
                                    <Plus className="h-4 w-4" />
                                    Adicionar Alimento / Prato
                                </button>
                            </div>
                        )
                    )}
            ...
            <div className="fixed bottom-28 left-0 right-0 z-40 px-4">
                <div className="mx-auto max-w-lg">
                    <Button
                        className="w-full bg-emerald-500 text-black font-black uppercase italic h-14 rounded-2xl hover:bg-emerald-600 shadow-2xl shadow-emerald-500/20 text-base gap-2 transition-all"
                    ...
                    >
                        ...
                        {isEdit ? "Salvar alterações" : "Salvar Dieta"}
                    </Button>
                </div>
            </div>
