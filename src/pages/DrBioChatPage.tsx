...
const CHAT_STORAGE_PREFIX = "biotreiner_chat_v2_" as const;
...
const DrBioChatPage = () => {
    ...
    const streamDrBioChat = async (historyForApi: { role: "user" | "assistant"; content: string }[]) => {
        ...
        const response = await fetch(`${supabaseUrl}/functions/v1/dr-bio-chat`, {
            method: "POST",
            headers: {
    ...
    return (
        <>
            <main className="flex h-screen flex-col bg-[#0b141a]"> {/* WhatsApp Dark Background */}
                {/* WhatsApp Header */}
                <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between bg-[#202c33] px-2 py-2 shadow-sm">
                    <div className="flex items-center gap-1">
                        <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => navigate("/aluno/nutricionista")}
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex-1 bg-[#2a3942] rounded-2xl flex items-end px-3 min-h-[42px] mb-1 py-1.5">
                            <textarea
                                value={input}
                                onChange={(e) => {
                                    ...
                                    e.target.style.height = e.target.value ? `${Math.min(e.target.scrollHeight, 120)}px` : '24px';
                                }}
                                onKeyDown={(e) => {
                                ...
                                rows={1}
                                className="flex-1 bg-transparent border-0 shadow-none ring-0 focus:ring-0 focus-visible:ring-0 focus-visible:ring-offset-0 outline-none px-0 text-[#e9edef] placeholder:text-[#8696a0] text-[15px] resize-none overflow-y-auto w-full leading-5 m-0 p-0"
                                style={{
                                    height: '24px',
                                    maxHeight: '120px',
                                    minHeight: '24px',
                            ...
                            />
                            <div className="flex items-center gap-3 ml-2 shrink-0 pb-0.5">
