...
export const LojaFloatingNavIsland = () => {
  const navigate = useNavigate();
  const location = useLocation();

  const isActive = (path: string) => {
    if (path === "/loja/dashboard") {
      return location.pathname === path || location.pathname === "/loja";
    }
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex min-h-[48px] min-w-[48px] flex-col items-center justify-center gap-1 rounded-xl px-2 transition-all duration-200",
                active
                  ? "text-primary"
